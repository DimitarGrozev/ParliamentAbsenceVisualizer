...
export function AppProvider({ children }: AppProviderProps) {
  ...
  // Refetch absences with new date range and optional member name filter
  const refetchAbsences = async (startDate: string, endDate: string, memberName: string = '') => {
    try {
      setLoading(true);
      setError(null);

      // Fetch new absences with the specified date range and member name
      const rawAbsences: Absence[] = await fetchAbsences({
        search: 1,
        date1: startDate,
        date2: endDate,
        A_ns_MP_Name: memberName,
      });
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to refetch absences';
      setError(errorMessage);
      console.error('Error refetching absences:', err);
    } finally {
      setLoading(false);
    }
  };
